...
int evaluate_formula(Formula const &formula, int old) {

	int arg2 = (formula.m_type_2 == ArgumentType::CurrentValue) ? old : formula.m_value_2;
...
void run_monkey_turns(vector<Monkey> &monkeys) {

	for (auto &monkey: monkeys) {

		for (auto item: monkey.m_items) {
			int new_value = evaluate_formula(monkey.m_formula, item);
